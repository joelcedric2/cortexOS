...
describe("cronHistory", () => {
  test("returns job + run history", () => {
    const db = new CronJobsDB();
    const row = cronCreate(db, validInput);
    db.markRan(row.id, "success", 120, "ok");
    db.markRan(row.id, "fail", 500, "boom");
    const h = cronHistory(db, row.id);
    assert.equal(h.job.id, row.id);
    assert.equal(h.runs.length, 2);
    assert.equal(h.runs[0]?.outcome, "success");
    assert.equal(h.runs[0]?.duration_ms, 120);
    assert.equal(h.runs[1]?.outcome, "fail");
