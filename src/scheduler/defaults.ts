 ...
 * handle, which makes it trivially unit-testable with the stub DB.
 */
import type { CronJobInput, CronJobsDB } from "./_cron-jobs-db-stub.js";
 ...
 */
export const DEFAULT_JOBS: readonly CronJobInput[] = Object.freeze([
  {
    name: "morning_brief",
    cron_expr: "0 8 * * *",
...
export function seedDefaults(db: CronJobsDB): SeedResult {
  const existing = new Set(db.listAll().map((j) => j.name));
