...
import { nextRunFromCron } from "../scheduler/next-run.js";
...
export class NchindaTools {
  ...
  async schedule(raw: unknown): Promise<ScheduleResult> {
    ...
    const id = `cron_${randomUUID().slice(0, 12)}`;
    const nextRun = nextRunFromCron(parsed.cron_expr, this.deps.now?.() ?? new Date());
    const job = this.deps.cronDb.create({
      id,
      name,
